...
const TabSupport: React.FC = () => {
  const [messages, setMessages] = useState<ChatMessage[]>([
    { role: 'model', text: 'Assalomu alaykum! Men T TOP AI yordamchisiman. Sizga qanday yordam bera olaman?' }
  ]);
    ...
    <div className="flex flex-col h-[calc(100vh-140px)] animate-fade-in bg-white rounded-2xl overflow-hidden shadow-sm border border-gray-100">
      ...
      <div className="p-4 bg-gray-50 border-b border-gray-100 flex items-center justify-between">
        <div className="flex items-center gap-3">
            <div className="w-10 h-10 rounded-full bg-gradient-to-br from-primary to-purple-600 flex items-center justify-center text-white shadow-sm">
         ...
         <div className="px-4 py-2 bg-gray-50 border-t border-gray-100 overflow-x-auto no-scrollbar flex gap-2">
            {SUGGESTIONS_UZ.map((faq, i) => (
               <button 
                  ...
                  className="whitespace-nowrap px-3 py-1.5 bg-white border border-gray-200 rounded-full text-xs text-primary hover:bg-purple-50 transition-colors"
               >
                  {faq}
               </button>
            ))}
